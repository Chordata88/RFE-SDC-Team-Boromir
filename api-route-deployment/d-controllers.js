...
const reportQuestion = async (id, callback) => { await database.Questions.update({question_reported: 1}, {where: {question_id: id}}).then(() => {callback('reported successfully')}).catch((err) => {callback(err)}) };
...
const addQuestion = async (body, callback) => { //console.log(body, "in controller");
  ...
  let obj = { question_id: q_id,
    ...
    asker_email: body.email, question_helpfulness: 0, question_reported: 0
